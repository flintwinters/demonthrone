...
const pushableField = new PerlinSpawnField(new NoiseLayer({ scale: 0.17, seed: 0x63726174 }), 0.7);
const enemyField = new PerlinSpawnField(new NoiseLayer({ scale: 0.1, seed: 0x6e6d79 }), 0.65);
export function materializeEntities(units, enemies) {
    const pushableTiles = pushableField.materialize(units, generationRadius, (tile) => isAvailable(tile, units, enemies));
    pushables.push(...pushableTiles.map((tile) => createPushable(`crate@${tileKey(tile)}`, tile)));
    const enemyTiles = enemyField.materialize(units, generationRadius, (tile) => isAvailable(tile, units, enemies));
